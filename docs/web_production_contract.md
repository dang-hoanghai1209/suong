...
- optional `TELLA_WEB_MIN_FREE_BYTES`, default `2147483648` (2 GiB)
...
The server measures web-job storage without following symlinks or reparse
points. `GET /api/storage` reports output-root and per-job byte counts, free
volume bytes, the configured minimum, and submission availability without
revealing the absolute output path. New submissions fail with
`INSUFFICIENT_DISK_SPACE` before a job record or directory is created when free
space is below the minimum; equality is allowed.

Retention is non-destructive by default: completed MP4 files are never removed
automatically. A terminal job may be compacted explicitly from the UI after a
confirmation prompt. Compaction retains `job.json` and a validated published
MP4 while removing only known reconstructable assets, narration intermediates,
scene renders, and publication-input metadata. Queued/running jobs, traversal,
symlinks, reparse points, and unknown top-level entries are rejected. Repeated
compaction is safe, and restart preserves publication.

On restart, persisted queued and running records from an earlier server
instance fail closed with `SERVER_RESTART_INTERRUPTED`; they are never resumed
or executed automatically. This preserves at-most-once execution and requires
an explicit retry, which creates one idempotent successor job under current
readiness, storage, worker, privacy, and provider policy. Last-known phase and
progress remain diagnostic, partial MP4 files remain unpublished, and only an
already validated succeeded record is revalidated for publication.

Each process worker is bound to the creating server instance and a private
parent-owned pipe. Parent-control EOF revokes the worker while a render is in
progress and terminates only that worker's process tree. Persisted ownership is
diagnostic only: a restarted server never signals a numeric PID from
`job.json`. Corrupt, truncated, invalid-UTF-8, or oversized metadata is bounded
before parsing and atomically quarantined inside its canonical job directory;
it cannot authorize preview, download, compaction, or execution.

For student laptops, keep the 2 GiB default or raise it to reserve space for
other applications. Lower it only after checking expected MP4 and intermediate
asset sizes. Compacted intermediates must be regenerated by a new production if
later editing is required; the validated MP4 remains available.
